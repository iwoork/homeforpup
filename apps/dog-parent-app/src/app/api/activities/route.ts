...
import { db, activities, eq, and, gte, lte, inArray, desc, sql } from '@homeforpup/database';
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (filters.dateRange) {
      conditions.push(gte(activities.timestamp, filters.dateRange.start));
      conditions.push(lte(activities.timestamp, filters.dateRange.end));
    }
    ...
    const typedActivities = allActivities as unknown as Activity[];

    // Apply pagination
    const startIndex = filters.offset || 0;
    const endIndex = startIndex + (filters.limit || 20);
    const paginatedActivities = typedActivities.slice(startIndex, endIndex);
    ...
    const response: ActivityResponse = {
      ...
      total: typedActivities.length,
      hasMore: endIndex < typedActivities.length,
      stats,
    };
