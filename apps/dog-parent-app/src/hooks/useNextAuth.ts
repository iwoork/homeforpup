...
export const useNextAuth = (): {
  ...
  session: any;
  activeProfileType: 'breeder' | 'dog-parent';
  switchProfileType: (profileType: 'breeder' | 'dog-parent') => void;
  ...
  const [activeProfileType, setActiveProfileType] = useState<'breeder' | 'dog-parent'>('dog-parent');
  ...
  useEffect(() => {
    if (user) {
      ...
      } else if (user.userType === 'dog-parent') {
        setActiveProfileType('dog-parent');
      } else if (user.userType === 'both') {
        // Load from localStorage or default to 'dog-parent'
        const savedProfileType = localStorage.getItem('activeProfileType') as 'breeder' | 'dog-parent' | null;
        setActiveProfileType(savedProfileType || 'dog-parent');
      }
    }
  }, [user]);

  // Save profile type selection to localStorage
  const switchProfileType = useCallback(async (profileType: 'breeder' | 'dog-parent') => {
