...
const EditLitterScreen: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    availablePuppies: litter.availablePuppies?.toString() || '',
    priceMin: litter.priceRange?.min.toString() || '',
    priceMax: litter.priceRange?.max.toString() || '',
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await apiService.updateLitter(litter.id, litterUpdate);

      if (response.success) {
        Alert.alert('Success', 'Litter updated successfully!', [
          {
            text: 'OK',
      ...
      Alert.alert(
        'Error',
        error instanceof Error ? error.message : 'Failed to update litter. Please try again.',
      );
    }
  };
  ...
  return (
    <View style={styles.keyboardAvoidingView}>
      ...
      >
        ...
        <Icon name="create" size={48} color={theme.colors.primary} />
        <Text style={styles.headerTitle}>Edit Litter</Text>
        <Text style={styles.headerSubtitle}>Update litter information</Text>
      </View>
...
export default EditLitterScreen;
