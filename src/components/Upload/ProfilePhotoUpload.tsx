        ...
        onCropped={async (blob) => {
          try {
            ...
            try {
              ...
              console.log('ProfilePhotoUpload: Uploaded to S3, URL:', url);
            } catch (s3Error) {
              console.error('S3 upload failed, using data URL fallback:', s3Error);
              // Fallback to data URL for testing
              url = URL.createObjectURL(blob);
              console.log('ProfilePhotoUpload: Using data URL fallback:', url);
            }
            
            console.log('ProfilePhotoUpload: Calling onChange with URL:', url);
            onChange?.(url);
